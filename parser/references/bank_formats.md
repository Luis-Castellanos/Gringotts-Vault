...
**✅ RESOLVED — loan-ledger model = balance_snapshots (no transactions):** the mortgage statement contributes only its stated **unpaid principal** as a `balance_snapshots` row (`ingestBalanceSnapshot`, idempotent per account+as_of, source='statement'), NOT ledger transactions. `buildMortgage` (lib/properties/load.ts) now prefers the latest snapshot over the amortization estimate → the Real Estate mortgage balance is statement-accurate. This deliberately avoids: (a) Net-Worth corruption (principal-only transactions with no opening balance would show the mortgage as a tiny asset), and (b) double-counting the checking-side mortgage split (which already moves principal→loan / interest→expense / escrow→escrow). The principal/interest/escrow breakdown stays on the statement for reference; chase_mortgage is in POPPLER_LAYOUT_ISSUERS so the value extracts cleanly (Xpdf mangled the two-column page). Chase Auto's only sample is a welcome letter (no balance yet).
...
- **`ally`** + **`schwab_checking`** — ✅ **now parsing via balance-delta signing** (`_parse_balance_delta`). They print debits in a separate column with no reliable inline minus, so the signed amount = running-balance[i] − [i−1] (correct regardless of column layout), reset at each account section's `Beginning Balance` (Ally's statement is combined savings+checking). Validated: net = ending − beginning on every sample (Ally $0.43, Schwab $0.20), signs + dates correct. Schwab period is "Month D-D, YYYY" (monthly) or "Month D, YYYY to Month D, YYYY" (quarterly) — `_schwab_period` handles both (the year bug was the monthly format falling back to `now().year`=2026). Detect: Ally = "ALLY BANK"+"CUSTOMER STATEMENT"; Schwab = "SCHWAB BANK"/"CHARLES SCHWAB".
- **Wealthfront** = Green Dot Bank cash account — not yet built (format TBD).

## Optum HSA (cash) + Wealthfront + the empty folders (added 2026-05-25)

- **`optum_hsa`** — ✅ parsing as a **cash bank account** (moved out of INVESTMENT_ISSUERS; the HSA's investment side is $0). Rows: `M/D/YYYY <desc> <deposits$> <withdrawals$> <balance$>` — amount = deposits − withdrawals (separate columns, no inline minus; rows are newest-first but column math is order-independent). Validated: net = ending − beginning (e.g. −$183.32 = $183.34 − $366.66). Both #5229 + #6476 (17 stmts).
- **`wealthfront`** (Green Dot Bank) — recognized + **deferred**. The Green Dot statement is structurally empty ("No Transactions"): funds sweep to a separate *Wealthfront Cash Account* daily, so begin/end are $0.00. The real activity is in a Wealthfront Cash Account statement not present in OneDrive.
- **Apple Cash** + **Fidelity CMA #1931** — folders exist but contain **0 PDFs** (no statements downloaded). Nothing to parse.
